...
const toolMap = {
  ...
  get_x_narrative: getXNarrativeSignal,
  ...
  update_config: ({ changes, reason = "" }) => {
    ...
    const CONFIG_MAP = {
      ...
      screeningTimeframe: ["screening", "timeframe"],
      entryTimeframe: ["screening", "entryTimeframe"],
      ...
      xNarrativeFreshHours: ["screening", "xNarrativeFreshHours"],
      minPoolTvl: ["screening", "minPoolTvl"],
      minPoolOpenPositions: ["screening", "minPoolOpenPositions"],
      minFeePerTvl24h: ["management", "minFeePerTvl24h"],
      bandDeployEnabled: ["management", "bandDeployEnabled"],
      bandBSizeMultiplier: ["management", "bandBSizeMultiplier"],
      // management
      ...
      gmgnPoolDetailTimeframe: ["gmgn", "poolDetailTimeframe"],
      gmgnEntryTimeframe: ["gmgn", "entryTimeframe"],
...
export async function executeTool(name, args) {
  ...
  try {
    if (name === "deploy_position") {
      let provided = args.signal_snapshot;
      if (typeof provided === "string") {
        try {
          const p = JSON.parse(provided);
          provided = p && typeof p === "object" && !Array.isArray(p) ? p : null;
        } catch { provided = null; }
      } else if (provided != null && (typeof provided !== "object" || Array.isArray(provided))) {
        provided = null;
      }
      const rebuilt = {
        gmgn_score: args.gmgn_score ?? null,
        fee_active_tvl_ratio: args.fee_tvl_ratio ?? args.fee_active_tvl_ratio ?? null,
        volatility: args.volatility ?? null,
        holders: args.holders ?? null,
        mcap: args.mcap ?? null,
        organic_score: args.organic_score ?? null,
        fragility_level: args.fragility_level ?? null,
        fragility_score: args.fragility_score ?? null,
        lpagent_confidence: args.lpagent_confidence ?? null,
        smart_wallets_present: args.smart_wallets_present ?? null,
        smart_wallet_count: args.smart_wallet_count ?? null,
        discord_active: args.discord_active ?? null,
        narrative_confidence: args.narrative_confidence ?? null,
        x_narrative_score: args.x_narrative_score ?? null,
        token_age_hours: args.token_age_hours ?? null,
        screening_band: args.band ?? null,
      };
      args = { ...args, signal_snapshot: { ...rebuilt, ...(provided || {}) } };
    }
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        ...
        // Handle signal_snapshot - may be object, string, or missing
        let snap = args.signal_snapshot || {};
        if (typeof snap === "string") {
          try { snap = JSON.parse(snap); } catch { snap = {}; }
        }
        if (typeof snap !== "object" || Array.isArray(snap)) snap = {};
        const funnelReasons = Array.isArray(snap.funnel_reasons) ? snap.funnel_reasons : [];
        const funnelRisks = Array.isArray(snap.funnel_risks) ? snap.funnel_risks : [];
        const whyParts = [
          result.band ? `band ${result.band}` : args.band ? `band ${args.band}` : null,
          ...funnelReasons.slice(0, 3),
          funnelRisks.length ? `risks: ${funnelRisks.slice(0, 2).join("; ")}` : null,
          snap.narrative_confidence != null ? `x=${snap.narrative_confidence}` : args.narrative_confidence != null ? `x=${args.narrative_confidence}` : null,
          snap.active_tvl != null ? `active TVL $${Math.round(Number(snap.active_tvl))}` : null,
          snap.open_positions != null ? `open LPs ${snap.open_positions}` : null,
          snap.fee_active_tvl_ratio != null ? `fee/TVL ${snap.fee_active_tvl_ratio}%` : args.fee_tvl_ratio != null ? `fee/TVL ${args.fee_tvl_ratio}%` : null,
          snap.mcap != null ? `mcap $${Math.round(Number(snap.mcap))}` : args.mcap != null ? `mcap $${args.mcap}` : null,
          snap.token_age_hours != null ? `age ${snap.token_age_hours}h` : args.token_age_hours != null ? `age ${args.token_age_hours}h` : null,
        ];
        const why = whyParts.filter(Boolean).join(" | ");
        notifyDeploy({ pair: result.pool_name || args.pool_name || args.pool_address?.slice(0, 8), amountSol: result.amount_y ?? args.amount_y ?? args.amount_sol ?? 0, position: result.position, tx: result.txs?.[0] ?? result.tx, priceRange: result.price_range, rangeCoverage: result.range_coverage, binStep: result.bin_step, baseFee: result.base_fee, why, band: result.band || args.band || null }).catch(() => {});
...
async function runSafetyChecks(name, args) {
  switch (name) {
    case "deploy_position": {
      ...
      // Enforce SOL-only deploys — amount_x > 0 is rejected
      ...
      let snapshotVolatility = null;
      if (typeof args.signal_snapshot === "object" && args.signal_snapshot !== null) {
        snapshotVolatility = args.signal_snapshot.volatility;
      ...
      if (rawVolatility != null && (!Number.isFinite(requestedVolatility) || requestedVolatility < 0)) {
        return {
          pass: false,
          reason: `volatility ${rawVolatility} is invalid. Refusing deploy because the volatility feed is unusable.`,
        };
      }
      const maxVolatilityHard = Number(config.screening.maxVolatilityHard);
      if (
        rawVolatility != null &&
        Number.isFinite(requestedVolatility) &&
        Number.isFinite(maxVolatilityHard) &&
        requestedVolatility > maxVolatilityHard
      ) {
        return {
          pass: false,
          reason: `volatility ${requestedVolatility} is above hard maximum ${maxVolatilityHard}. Refusing deploy; entry is too unstable for DLMM LP.`,
        };
      }
      if (
      ...
      if (
        deployAmountY > 0 && deployAmountX <= 0 &&
        args.downside_pct == null &&
