...
Current screening/source timeframe: ${config.screening.timeframe} — GMGN rank volume and Meteora pool economics are interpreted relative to this window.
Current entry signal timeframe: ${config.screening.entryTimeframe ?? config.gmgn?.entryTimeframe ?? "5m"} — use lower-timeframe indicator confirmation for entry timing, not for high-level candidate selection.
...
Each candidate includes a funnel band plus concise reasons/risks. Deploy only band A or band B candidates.
...
- Sizing is decided by the runtime. Do NOT pass amount_y larger than the goal value.
...
- Pick ONE pool. Prefer the strongest band-A candidate; use band B only when its reasons still clear the hard gates.
