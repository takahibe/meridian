...
async function fetchPoolDetailDirect(poolAddress, timeframe = config.gmgn?.poolDetailTimeframe ?? config.screening?.timeframe ?? "1h") {
  ...
  // Pool-detail timeframe is intentionally separate from the entry signal timeframe:
  // use the screening/source window (e.g. 1h) for volume/fee/TVL filtering, then
  // use the lower entry timeframe (e.g. 5m) in Stage 4 indicators.
  const discoveryBase = "https://pool-discovery-api.datapi.meteora.ag";
  const detailTimeframe = normalizeInterval(timeframe, "1h");
  const url = `${discoveryBase}/pools?page_size=1&filter_by=${encodeURIComponent(`pool_address=${poolAddress}`)}&timeframe=${detailTimeframe}`;
  const res = await fetch(url);
