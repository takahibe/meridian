...
export async function deployPosition({
  ...
  const recommendedBinsBelow = cached?.bins ?? (volatility == null ? config.strategy.minBinsBelow : computeBinsBelow(volatility, null));
  ...
  // Enforce minimum safe range — never deploy fewer than MIN_SAFE_BINS_BELOW total bins
  activeBinsBelow = Math.max(MIN_SAFE_BINS_BELOW, activeBinsBelow);
  activeBinsAbove = Math.max(0, activeBinsAbove);
  ...
  const stagedSignalSnapshot = getAndClearStagedSignals(pool_address);
  const incoming = (signal_snapshot && typeof signal_snapshot === "object" && !Array.isArray(signal_snapshot))
    ? signal_snapshot
    : {};
  const resolvedSignalSnapshot = { ...(stagedSignalSnapshot || {}), ...incoming };

  if (process.env.DRY_RUN === "true") {
    const totalBins = activeBinsBelow + activeBinsAbove;
    const requestedAmountY = amount_y ?? amount_sol ?? 0;
    ...
    return {
      ...
      would_deploy: {
        ...
        requested_amount_y: requestedAmountY,
  ...
  const fallbackAmountY =
    amount_y == null && amount_sol == null
      ...
      : 0;
  const requestedAmountY = amount_y ?? amount_sol ?? fallbackAmountY;
  const sizeMultiplier = band === "B" && config.management.bandDeployEnabled
    ? Number(config.management.bandBSizeMultiplier || 0.5)
    : 1;
  const finalAmountY = Number((requestedAmountY * sizeMultiplier).toFixed(6));
  if (band) {
    log("deploy", `screening band=${band} size_mult=${sizeMultiplier} requested=${requestedAmountY} final=${finalAmountY}`);
  }
